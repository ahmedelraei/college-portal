...
export class CoursesService {
  ...
  // GraphQL input handling methods
  async createFromGraphQL(
    createCourseInput: CreateCourseInput,
  ): Promise<Course> {
    const createCourseDto: CreateCourseDto = {
      courseCode: createCourseInput.courseCode,
      courseName: createCourseInput.courseName,
      description: createCourseInput.description,
      creditHours: createCourseInput.creditHours,
      pricePerCredit: createCourseInput.pricePerCredit || 500.0,
      semester: createCourseInput.semester,
      prerequisiteIds: createCourseInput.prerequisiteIds?.map((id) =>
        id.toString(),
      ),
    };
    return this.create(createCourseDto);
  }

  async updateFromGraphQL(
    id: number,
    updateCourseInput: UpdateCourseInput,
  ): Promise<Course> {
    const updateCourseDto: UpdateCourseDto = {
      courseCode: updateCourseInput.courseCode,
      courseName: updateCourseInput.courseName,
      description: updateCourseInput.description,
      creditHours: updateCourseInput.creditHours,
      pricePerCredit: updateCourseInput.pricePerCredit,
      semester: updateCourseInput.semester,
      prerequisiteIds: updateCourseInput.prerequisiteIds?.map((id) =>
        id.toString(),
      ),
      isActive: updateCourseInput.isActive,
    };
    return this.update(id, updateCourseDto);
  }

  async findAllFromGraphQL(filters?: CourseFilterInput): Promise<Course[]> {
    const courseFilterDto: CourseFilterDto = {
      search: filters?.search,
      creditHours: filters?.creditHours,
      semester: filters?.semester,
    };
    return this.findAll(courseFilterDto);
  }
}
